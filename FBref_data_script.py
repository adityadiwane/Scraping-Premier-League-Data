...
def animate():
    for c in itertools.cycle(['|', '/', '-', '\\']):
        if done:
            break
        sys.stdout.write('\rDownloading Football Data ' + c)
        sys.stdout.flush()
        time.sleep(0.1)
    sys.stdout.write('\rFootball Data Downloaded    \n')
...
#standard(stats)
leagues = ["games","wins","draws","losses","goals_for","goals_against",
          "goal_diff","points","xg_for","xg_against","xg_diff",
          "xg_diff_per90"]
league = ["games_home","wins_home","draws_home","losses_home","goals_for_home","goals_against_home",
          "goal_diff_home","points_home","points_avg_home","xg_for_home","xg_against_home","xg_diff_home",
          "xg_diff_per90_home","games_away","wins_away","draws_away","losses_away","goals_for_away",
          "goals_against_away","goal_diff_away","points_away","points_avg_away","xg_for_away","xg_against_away",
          "xg_diff_away","xg_diff_per90_away"]
...
def get_frame_team_league(features, team_table):
    pre_df_squad = dict()
    #Note: features does not contain squad name, it requires special treatment
    features_wanted_squad = features
    rows_squad = team_table.find_all('tr')
    for row in rows_squad:
        if(row.find('th',{"scope":"row"}) != None):
                rank = row.find('th',{"data-stat":"rank"}).text.strip().encode().decode("utf-8")
                name = row.find('td',{"data-stat":"squad"}).text.strip().encode().decode("utf-8")
                if 'squad' and 'rank' in pre_df_squad:
                    pre_df_squad['rank'].append(rank)
                    pre_df_squad['squad'].append(name)
                else:
                    pre_df_squad['rank'] = [rank]
                    pre_df_squad['squad'] = [name]
                for f in features_wanted_squad:
                    cell = row.find("td",{"data-stat": f})
                    a = cell.text.strip().encode()
...
def frame_for_category_team1(top,end,features):
    url = (top + end)
    player_table, team_table = get_tables(url)
    df_team = get_frame_team_league(features, team_table)
    return df_team

#################################################### Get Outfield Player Data ####################################
...
def get_league_table_data(top,end): 
    df = frame_for_category_team1(top, end, leagues)
    df = df.loc[:,~df.columns.duplicated()]
    return df
...
df_outfield.to_csv('PL2021_Outfield.csv',index=False,encoding='utf-8')
...
#Save csv file to Desktop
df_team.to_csv('PL2021_teams.csv',index=False,encoding='utf-8')

#Get League Table Data

df_league_table = get_league_table_data('https://fbref.com/en/comps/9/','/Premier-League-Stats')

#Save csv file to Desktop
df_league_table.to_csv('PL2021_league_table.csv',index=False,encoding='utf-8')
...
#Save csv file to Desktop
df_team.to_csv('PL2021_keepers.csv',index=False,encoding='utf-8')
